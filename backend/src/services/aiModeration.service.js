  ...
  getThresholds() {
    if (this.STRICT_MODE) {
      return {
        ...
        imageTextMatch: 0.65,
        individualScore: 0.8,
        imageConsistency: 0.75,
        vietnameseLanguage: 0.75,
        legalCompliance: 0.0
      };
    }
    return {
...
function estimateProductWeight(productName, productDescription = "") {
  ...
  const finalFallbackWeight = Math.max(Math.round(fallbackWeight), 20);

  return {
    estimatedWeight: finalFallbackWeight,
    confidence: 0.25,
    reasoning:
    "Fallback estimation based on text analysis and material detection",
    range: {
      min: Math.max(Math.round(finalFallbackWeight * 0.6), 15),
      max: Math.round(finalFallbackWeight * 1.8)
    },
    sizeAdjustment: null,
    unit: "grams",
    category: "unknown"
...
function detectGarbledText(text) {
  ...
  /[a-zA-Zàáạảãâầấậẩẫăằắặẳẵèéẹẻẽêềếệểễìíịỉĩòóọỏõôồốộổỗơờớợởỡùúụủũưừứựửữỳýỵỷỹđ]{2,}[dfgh]{2,}[a-zA-Zàáạảãâầấậẩẫăằắặẳẵèéẹẻẽêềếệểễìíịỉĩòóọỏõôồốộổỗơờớợởỡùúụủũưừứựửữỳýỵỷỹđ]{1,}/g,
  /[a-zA-Z]{1,}[dfgh]{1,}[gf]{1,}[a-zA-Z]{0,}/g,
  /\b[a-zA-Z]*[dfgh]{2,}[a-zA-Z]*\b/g,
  /\b[a-zA-Z]+[fd][gd][a-zA-Z]*\b/g,
  /[a-zA-Z]{1,}[fd][a-zA-Z]{0,2}[gd][a-zA-Z]{0,2}/g,
  /\b\w*[dfg]{3,}\w*\b/g];
  ...
  garbledPatterns.forEach((pattern, index) => {
    ...
    if (matches && matches.length > 0) {
      found.push({ pattern: index, matches: matches.slice(0, 3) });
    }
  });
...
function assessVietnameseCoherence(text) {
  ...
  words.forEach((word) => {
    const cleanWord = word.replace(/[^\wăâêôơuưy]/g, "");
  ...
  score -= suspiciousRatio * 2;
  score -= invalidRatio * 1.5;
...
async function performSpecializedAIAnalysisOld(text) {
  const apiKeys = validateAPIKeys();
  const results = {
    bannedProductScore: 0.0,
    vietnameseQualityScore: 0.5,
    aiProvidersUsed: [],
    errors: []
  };
  ...
  if (apiKeys.google) {
    try {
      ...
      results.bannedProductScore = 0.3;
      results.vietnameseQualityScore = 0.4;
    }
  } else {
...
Bạn là chuyên gia phân tích hình ảnh sản phẩm CỰC KỲ NGHIÊM NGẶT. Phân tích ${
        validImages.length} hình ảnh để kiểm tra:

THÔNG TIN SẢN PHẨM:
- Tên: ${
        prompt.match(/TÊN SẢN PHẨM: (.+)/)?.[1] || "N/A"}
...
productDescription = "")
{
  ...
  if (!captions.length || !productTitle) return 0.1;
  ...
  if (validCaptions.length === 0) return 0.15;
  ...
  if (productKeywords.size === 0) return 0.1;
  ...
  const criticalBonus = criticalMatches > 0 ? 0.1 : 0;

  const finalScore = Math.min(averageScore + criticalBonus, 1.0);


  return Math.max(finalScore, 0.05);
}
...
async function performEmergencyModeration(productId, productData, startTime) {
  try {
    ...
    const emergencyApproved = false;
    const confidence = Math.min(emergencyScore, 0.85);
...
async function processEnhancedAIModerationBackground(productId, productData) {
  ...
  try {
    ...
    console.log(
      `${
      approved ? "✅" : finalDecision === "NEEDS_REVIEW" ? "🔍" : "❌"} Enhanced AI moderation complete: ${
      status.toUpperCase()} | Product: ${productId} | Time: ${processingTime}ms | Confidence: ${confidence.toFixed(
        3
...
function getModerationSystemHealth() {
  ...
  const hasSpecializedAI = apiKeys.google && apiKeys.groq;
