...
function fire(io, userId, opts) {
  ...
  return notifyUser({ io, userId, ...opts }).catch(
    (e) => console.error("[NotificationService]", e.message),
  );
}
...
const NotificationService = {
  ...
  async orderStatusChanged({ io, order, newStatus }) {
    const id = sid(order);
    const msgs = {
      confirmed:        { title: "Đơn hàng đã xác nhận",        message: `Người bán đã xác nhận đơn #${id}.` },
      picked_up:        { title: "Đơn hàng đã được lấy hàng",   message: `Đơn hàng #${id} đã được lấy hàng.` },
      out_for_delivery: { title: "Đơn hàng đang giao",          message: `Đơn hàng #${id} đang trên đường đến bạn.` },
      delivered:        { title: "Đơn hàng đã giao",            message: `Đơn hàng #${id} đã được giao thành công.` },
      completed:        { title: "Đơn hàng hoàn tất",           message: `Đơn hàng #${id} đã hoàn tất.` },
      delivery_failed:  { title: "Giao hàng thất bại",          message: `Giao hàng cho đơn #${id} thất bại. Sẽ thử lại sớm.` },
      returning:        { title: "Đơn hàng đang được hoàn trả", message: `Đơn hàng #${id} đang được hoàn trả lại người bán.` },
      returned:         { title: "Đơn hàng đã hoàn trả",        message: `Đơn hàng #${id} đã hoàn trả về người bán.` },
    }[newStatus] ?? { title: "Cập nhật đơn hàng", message: `Đơn hàng #${id} cập nhật: ${newStatus}.` };
  ...
  async orderShipped({ io, order }) {
    ...
    return fire(io, order.buyerId, {
      ...
      link:    `/orders/${order._id}`,
      orderId: order._id,
      emailFn: () => sendOrderShippedEmail(buyer?.email, buyer?.fullName, order),
    });
  },
  ...
  async refundApproved({ io, order }) {
    ...
    return fire(io, order.buyerId, {
      ...
      link:    `/orders/${order._id}`,
      orderId: order._id,
      emailFn: () => sendRefundApprovedEmail(buyer?.email, buyer?.fullName, order),
    });
  },
  ...
  async payoutReleased({ io, order, netAmount }) {
    const id     = sid(order);
    const seller = await Account.findById(order.sellerId).select("email fullName").lean();
    return fire(io, order.sellerId, {
      type: "order", realtime: true, email: true,
      ...
      link:    "/seller/wallet",
      orderId: order._id,
      emailFn: () => sendPayoutReleasedEmail(seller?.email, seller?.fullName, order, netAmount),
    });
  },
};
